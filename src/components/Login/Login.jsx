...
// import GetmaxLogo from "../../assets/Getmaxlogobluecolor.png";
...
export default function Login() {
  ...
  const handleGoogleLogin = () => {
    // URL for Google login page
    const googleLoginUrl = "https://accounts.google.com/login";

    // Open Google login page in a popup window
    ...
    // Close the popup window after successful login (optional)
    const checkGoogleLogin = setInterval(() => {
      if (googleLoginWindow.closed) {
        clearInterval(checkGoogleLogin);
        // Perform actions after successful login
        // For example, fetch user data or redirect to another page
  ...
  const handleGitHubLogin = () => {
    // URL for GitHub login page
    const gitHubLoginUrl = "https://github.com/login/oauth/authorize";

    // Open GitHub login page in a popup window
    ...
    // Close the popup window after successful login (optional)
    const checkGitHubLogin = setInterval(() => {
      if (gitHubLoginWindow.closed) {
        clearInterval(checkGitHubLogin);
        // Perform actions after successful login
        // For example, fetch user data or redirect to another page
  ...
  //   // Replace {tenant} with your actual tenant ID or domain
  //   const tenantIdOrDomain = "{tenant}";
  //   const redirectUri = encodeURIComponent("YOUR_REDIRECT_URI");
  //   const clientId = "YOUR_CLIENT_ID";
  //   const scope = encodeURIComponent("openid profile email");

  //   const microsoftAuthUrl = `${microsoftLoginUrl}?client_id=${clientId}&response_type=code&redirect_uri=${redirectUri}&scope=${scope}&state=STATE_VALUE`;

  //   // Open Microsoft login page in a popup window
  ...
  //   // Close the popup window after successful login (optional)
  //   const checkMicrosoftLogin = setInterval(() => {
  //     if (microsoftLoginWindow.closed) {
  //       clearInterval(checkMicrosoftLogin);
  //       // Perform actions after successful login
  //       // For example, fetch user data or redirect to another page
 ...
 // Function to generate Microsoft redirect URI dynamically
const generateRedirectUri = () => {
  // Replace 'yourdomain.com' with your actual domain
  const domain = 'yourdomain.com';
  // Replace '/callback' with your actual callback route
  const callbackRoute = '/callback';
  return encodeURIComponent(`https://${domain}${callbackRoute}`);
};

// Function to handle Microsoft login
const handleMicrosoftLogin = () => {
  // URL for Microsoft authentication
  const microsoftLoginUrl = `https://login.microsoftonline.com/{tenant}/oauth2/v2.0/authorize?client_id={clientId}&response_type=code&redirect_uri=${generateRedirectUri()}&response_mode=query&scope={scopes}`;
  ...
  // Optional: Close the popup window after successful login
  const checkMicrosoftLogin = setInterval(() => {
      if (microsoftLoginWindow.closed) {
          clearInterval(checkMicrosoftLogin);
          // Perform actions after successful login
  ...
  const handleTwitterLogin = () => {
    ...
    const twitterLoginUrl = "https://api.twitter.com/oauth/authorize";

    // Open Twitter login page in a popup window
    ...
    // Close the popup window after successful login (optional)
    const checkTwitterLogin = setInterval(() => {
      if (twitterLoginWindow.closed) {
        clearInterval(checkTwitterLogin);
        // Perform actions after successful login
        // For example, fetch user data or redirect to another page
        ...
        <div className="xl:px-20 md:px-10 sm:px-6 px-4 md:py-12 py-9 2xl:mx-auto 2xl:container md:flex items-center justify-center">
          ...
          <div className="bg-white shadow-lg rounded xl:w-1/3 lg:w-5/12 md:w-1/2 w-full lg:px-10 sm:px-6 sm:py-10 px-2 py-6">
            ...
            <button
              aria-label="Continue with Microsoft"
              role="button"
              onClick={handleMicrosoftLogin} // Call function to handle Microsoft login
              className="focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-700 p-3 border rounded-lg border-gray-700 flex items-center w-full mt-4 hover:bg-gray-100"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <button
              aria-label="Continue with Google"
              role="button"
              onClick={handleGoogleLogin} // Call function to handle Google login
              className="focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-700 p-3 border rounded-lg border-gray-700 flex items-center w-full mt-4 hover:bg-gray-100"
            >
              <svg
                width={19}
            ...
            <button
              aria-label="Continue with GitHub"
              role="button"
              onClick={handleGitHubLogin} // Call function to handle GitHub login
              className="focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-700 p-3 border rounded-lg border-gray-700 flex items-center w-full mt-4 hover:bg-gray-100"
            >
              <svg
                width={21}
            ...
            <button
              aria-label="Continue with Twitter"
              role="button"
              onClick={handleTwitterLogin} // Call function to handle Twitter login
              className="focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-700 p-3 border rounded-lg border-gray-700 flex items-center w-full mt-4 hover:bg-gray-100"
            >
              {/* Twitter icon SVG */}
              <svg
